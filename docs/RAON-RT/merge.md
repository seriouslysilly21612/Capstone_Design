...
- **다음(즉시): park 자세와 IK 시드 분리** — 현재 HOME 기록이 앵커(계산용 시드)와 `b` 복귀
  목표(물리적 대기 자세)를 동시에 정하는데 두 요구가 상충한다(좋은 시드는 작업공간 근처,
  좋은 park는 카메라 시야 밖). 지금은 대기 자세가 물체를 가려 **초기에 락 잡히는 물체가
  제한됨**(오퍼레이터 관측). q_ready는 시드로 두고 `b`가 가는 카메라 회피 park를 별도로
