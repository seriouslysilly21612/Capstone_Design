...
`9204372` **안전 백스톱**(NaN 게이트+토크 새추레이션+CTC 스냅샷) → `4efb05d` **실시간 추적 'o'**(D15, 실기 검증 대기).
