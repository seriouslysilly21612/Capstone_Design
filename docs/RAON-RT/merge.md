...
- **Kd vs Ki 분석(2026-07-31, F/T 제거 A/B #24~26 + DataLog 관절 단위 판독)**:
  ① **A/B 판정(정정 2026-07-31: #24부터는 실물 센서+어댑터판도 물리적으로 탈거 — 모델과 하드웨어가
  양쪽 다 일관된 진짜 A/B였음)**: 페이로드(실물+모델)를 통째로 제거해도 **1차 미스 크기는 거의 불변**
  (banana 46→40 mm, mustard 33→26 mm), 방향만 회전(banana y +28→−23 mm 플립 = WITH 상태의 편향은
  페이로드 몫이었음) → **질량·중력 모델은 droop 크기의 주범이 아님이 확정**, stiction+손목 저권한
  결론이 강화됨. j4 shortfall −0.18→−0.25 악화는 물체 배치가 달라 IK가 손목 이동을 2배 요구한 탓
  (모델 문제 아님). **현 URDF(질량 0)는 현 하드웨어와 일치 = 올바른 상태**; 페이로드 모델 복원은
  **센서를 재장착할 때만**(그때 어댑터판 실측 + +Z CoM).
...
- **검증(#41~46, 07-31 02:00) + E35 — FF 소이동 과주(mustard 미스터리 해명, RAON `90e5409`)**:
  ...
  **원인 = j4 방향 비대칭**: 두 물체 모두 j4 **+방향**(반중력) 0.13~0.14 rad 요구 → act
  ...
  자세 무관).
  **🎯 원샷 목표 달성(#52~58, 07-31 02:41, KF_4=3.0)**: **7/7 refine 0회** — HOME 출발
  5/5(orange **33.8→7.7** / apple 10.7 / tennis **31.2→9.2** / banana 7.8 / mustard 7.0 mm)
  + **mustard→orange 전이도 원샷 11.3 mm**(#58; j0 −0.99 rad 대이동 99% 추종 = 출발 자세
  무관 실증). **KF_4 3.0이 반중력 +방향 해방**: #52 j4 +0.28 rad **100% 추종**(직전 0.005),
  #58 +0.31 rad 94%. **전 로그 과주 0.000 rad**(KF_4 2배 인상에도 lag gate가 상한 역할 실증),
  갭 7/7 0.0%. 남은 오차 구조가 바뀜: stiction 지배 → **바이어스 지배**(miss가 전 건
  −x 2~7/−z 3~9 mm로 일관 + j2(elbow) lag −0.014~−0.028 rad 상시 잔존 — apple 10.7 mm는
