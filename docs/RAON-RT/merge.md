...
- **다음(즉시) ①: reach map 툴** — `n/8` 코너 프로브를 실제 도달 영역 지도로 대체한다.
  앱과 동일하게 RBDL + `indy7.urdf`를 링크한 **헤드리스 오프라인 툴**(로봇·물리 불필요)로
  `SolveReadyIK` 로직을 그대로 복제(사다리·warm-start·관절한계·브랜치 갭·60° 게이트) →
  테이블 평면을 1 cm 격자로 훑어 CSV(`x,y,z,판정,이긴 w,tilt°,브랜치 갭,solve 수,ms`) →
  히트맵. 답해야 할 것: **머스타드가 얹혀 있는 브랜치 경계가 어디인지**(E27), 앵커를 바꾸면
  좋은 영역이 어떻게 넓어지는지, `n/8`을 무엇으로 대체할지
