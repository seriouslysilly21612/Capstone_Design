...
- **완료**: Phase 0(기반: RT-POSIX·EMasterApp aarch64) → Phase 1(버스 검증: 7슬레이브 PDO
  ...
  **Phase 5 전체**(손-눈 캘리브 rMc residual 1.8 cm → TF/박스 정본화 → **'v' 접근 데모
  ...
  자동 스테이징 — **실기 검증 대기**).
- **20시 실기**: ready-seed v3 정상 확인(괴랄 궤적 소멸, 전 접근 직행 FK 오차 9.6~11.9 mm)
  — 그런데 도달 위치가 물체를 ~11 cm 벗어남 → 프로브 판별로 **E21 확정: 사고 때 로봇
  베이스가 물리적으로 밀림**(카메라 대비 Δ(+6.3, +9.7) cm, z는 8 mm 일치 = 평면 강체 이동).
- **다음**: ① 베이스 볼트 고정(안전) ② 3점 프로브 강체 fit(yaw+평행이동) → launch TF 보정
  → apple 재검증 ③ 이후 파지 준비(TCP 재정의 +29 mm, <5 mm, top-down R, 물체 6D는 나중)
  — §9. task-space servo는 사고 부검(E17) 전제조건 하 백로그.
...
- **다음(즉시): E21 복구** — ① 베이스 고정 ② 3점 프로브 강체 fit → launch TF 보정(§7 E21)
  ③ apple 접근 재검증(물체 위 도달 확인)
