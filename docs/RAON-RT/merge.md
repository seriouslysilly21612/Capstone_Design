...
**★ 하드웨어 제약 — J2(팔꿈치) 회전 불가 (오퍼레이터 제보, 2026-07-28 03:45)**: 베이스를 J0로
셀 때 **J2에 lock이 걸려 회전이 안 됨**. 정확한 성격(기계적 완전 고정 / 손으로만 안 움직이고
서보로는 구동됨 / 드라이브 fault)은 **미확인**. 데이터상 두 번의 HOME 기록 사이에 J2가
−0.92 → −0.87 rad(**2.9°**) 변했으므로 완전 고정은 아닐 가능성. **파급이 큼**: RBDL IK는 6축이
전부 자유롭다고 가정하므로 **J2를 움직이라는 해를 계속 생성**한다(실제 로그에
`dq_max 0.11 rad (J2)`, `0.04 rad (J2)`). 그 해는 물리적으로 달성되지 않고 refine이 나머지
관절로 보상하려다 진동 → **E26 테니스 refine 진동의 유력 용의자**, 도달 가능 집합이 실제보다
넓게 계산된다는 점에서 **E25와도 연결 가능**. 완전 고정으로 확인되면 **IK 자유도에서 J2를
제외**(또는 큰 가중치로 고정)해야 하며, 그전까지 E25/E26 진단은 이 변수를 배제하고 해석하지 말 것.
