...
| `field_prediction_20260729.csv` | **2026-07-29 실기 검증 — 실행 전에 기록한 예측**(§7 E30) |
| `anchor_live-20260729.txt` | 그때의 앵커. 위 예측을 재현하려면 `--seed`로 이 파일을 지정할 것 |

## 2026-07-29 실기 검증 (다중 시드 IK, RAON `2e911a3`)

`field_prediction_20260729.csv`는 **로봇을 돌리기 전에** 기록한 예측이다. 목표
`(0.699, 0.357, 0.392)`에 대해:

| | 지도 예측 | 실기 |
|---|---|---|
| 기준선 | BRANCH 거부 | `J2 2.70 rad` 거부 |
| 다중시드 | PASS, w=0.00, seed 0, 8 solve, gap **0.39 rad (J2)**, tilt **29°**, 16.3 ms | PASS … gap **0.39 rad (J2)**, tilt **29°**, **14.92 ms** |

⚠️ 기준선의 거부 축은 지도 J5 2.97 / 실기 J2 2.70으로 **다르다**. 판정은 같지만
축·수치가 어긋나는데, 유력한 원인은 **앱이 찍는 좌표가 소수 3자리로 반올림된 것**이다
(실제 목표는 최대 0.5 mm 차이 — 이 구역은 1 mm 단위로 판정이 바뀌는 혼합 지대). 미확인.

**재생성**: `sweep_*`·`mustard_zoom_*`의 다중시드 판본은 병합된 툴로 한 명령이면 다시
만들어진다(`reachmap.out --seed <anchor> --step 0.01 --z ...`). 여기 보관된 것은
**되살릴 수 없는 쪽** — 병합 전 기준선 판본과, 실행 전에 기록한 예측이다.
